...
from torchir.regularization import bending_energy_2d
...
class NCC(_Loss):
    def __init__(self, use_mask: bool = False, bending_penalty: bool = False, alpha: float = None):
        super().__init__()
        self.bending_penalty = bending_penalty
        self.alpha = alpha
        if use_mask:
    ...
    def metric(self, fixed: Tensor, warped: Tensor, 
                batch_grid: Tensor = None, # Bending penalty parameters
                ) -> Tensor:

        if self.bending_penalty: 
            assert batch_grid is not None, "Bending penalty requires a batch of DVFs."
            l_ncc = -ncc(fixed, warped)
            l_bending = bending_energy_2d(batch_grid, vector_dim=1).nanmean()
            return l_ncc + self.alpha * l_bending
        return -ncc(fixed, warped)

    def masked_metric(self, fixed: Tensor, warped: Tensor, mask: Tensor,
                batch_grid: Tensor = None, # Bending penalty parameters
                ) -> Tensor:
        if self.bending_penalty: 
                assert batch_grid is not None, "Bending penalty requires a batch of DVFs."
                # print(torch.isnan(bending_energy_2d(batch_grid, vector_dim=1)).sum())
                return -ncc_mask(fixed, warped, mask) + self.alpha * bending_energy_2d(batch_grid, vector_dim=1).mean()
